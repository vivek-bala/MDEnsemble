		...
		self.data_transfer_task = bigjobasync.Task(
				name = "data-staging-task",
        			...
        			input = [
                				...
                				"mode" : bigjobasync.COPY,
                				"origin" : bigjobasync.LOCAL,
                				"origin_path" : self.task_info['source_directory']+self.task_info['pdbfile']
            					},
            					{ 
	                    			"mode"     : bigjobasync.COPY,  # copy it 
	                    			"origin_path"     : os.getcwd()+'/gromacs_bjasync/'+self.task_info['app_kernel'],
	                    			"origin" : bigjobasync.LOCAL, # file is on 'this' machine 
		                		},
	                			{
	                    			"mode"     : bigjobasync.COPY,   # ('LINK' will be a future option) 
	                    			"origin_path"     : self.task_info['source_directory']+self.task_info['runfile'],
	                    			"origin" : bigjobasync.LOCAL, # file is on 'this' machine 
	                			},
	                			{
	                			"mode"     : bigjobasync.COPY,   # ('LINK' will be a future option) 
	                    			"origin_path"     : self.task_info['source_directory']+self.task_info['emfile'],
	                    			"origin" : bigjobasync.LOCAL, # file is on 'this' machine 
		                		}
        				] 
        			)
		...
		for i in range(0,self.task_info['number_of_tasks']):
			self.gromacs_task = bigjobasync.Task(
				name        = "gromacs-task-%s" % i,
			        ...
			        executable  = "python",
            			arguments   = [
            					self.task_info['app_kernel'],
            					self.task_info['pdbfile'],
            					self.task_info['runfile'],
            					self.task_info['emfile'],
            					self.task_info['force_field'],
            					self.task_info['water_model']
            				],
	            		input = [
	                			{ 
	                    			"mode"     : bigjobasync.LINK,  # copy it 
	                    			"origin_path"     : self.task_info['pdbfile'],
	                    			"origin" : self.data_transfer_task, # file is on 'this' machine 
		                		},
		                		{ 
	                    			"mode"     : bigjobasync.LINK,  # copy it 
	                    			"origin_path"     : self.task_info['app_kernel'],
	                    			"origin" : self.data_transfer_task, # file is on 'this' machine 
		                		},
	                			{
	                    			"mode"     : bigjobasync.COPY,   # ('LINK' will be a future option) 
	                    			"origin_path"     : self.task_info['runfile'],
	                    			"origin" : self.data_transfer_task, # file is on 'this' machine 
	                			},
	                			{
	                			"mode"     : bigjobasync.COPY,   # ('LINK' will be a future option) 
	                    			"origin_path"     : self.task_info['emfile'],
	                    			"origin" : self.data_transfer_task, # file is on 'this' machine 
		                		}		 
	            			], 
	            		output = [
